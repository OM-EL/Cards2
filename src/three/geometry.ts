...
export function roundedRectFromDimensions(
  width: number,
  height: number,
  corners: number
) {
  ...
  shape.bezierCurveTo(
    -w,
    +h, // Control point should hit the real corner
    -w + c,
    +h, // Last two pairs are what lineTo would have been
    -w + c,
    +h
  ); // top left 2
  shape.lineTo(+w - c, +h); // top right 1
  shape.bezierCurveTo(+w, +h, +w, +h - c, +w, +h - c); // top right 2
  shape.lineTo(+w, -h + c); // bottom right 1
  shape.bezierCurveTo(+w, -h, +w - c, -h, +w - c, -h); // bottom right 2
  shape.lineTo(-w + c, -h); // bottom left 1
  shape.bezierCurveTo(-w, -h, -w, -h + c, -w, -h + c); // bottom left 2
...
export function TarotCardShape() {
  return roundedRectFromDimensions(2.75, 4.75, 0.125);
}
...
export function getDimensions(shape: THREE.Shape) {
  return shape.curves
    ...
    .map((x) => Math.abs(x[0]) + Math.abs(x[1])) as [number, number];
}
...
export function CardUVGenerator(shape: THREE.Shape, offset = [0, 0, 0, 0]) {
  ...
  return {
    generateTopUV: function (
      ...
      return [
        new THREE.Vector2(
          (ax - b[0][0]) / (w - (offset[0] + offset[2])),
          (ay - b[0][1]) / (h - (offset[1] + offset[2]))
        ),
        new THREE.Vector2(
          (bx - b[0][0]) / (w - (offset[0] + offset[2])),
          (by - b[0][1]) / (h - (offset[1] + offset[2]))
        ),
        new THREE.Vector2(
          (cx - b[0][0]) / (w - (offset[0] + offset[2])),
          (cy - b[0][1]) / (h - (offset[1] + offset[2]))
        )
      ];
    },
